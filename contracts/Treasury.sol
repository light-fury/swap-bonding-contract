...
import "./library/TrustSwapAccessControlled.sol";

contract TrustSwapTreasury is TrustSwapAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    IERC20Extended public immutable SWAP;
    // IsSWAP public sSWAP;
    ...
    constructor(
        ...
        uint256 _timelock,
        address _authority
    ) TrustSwapAccessControlled(ITrustSwapAuthority(_authority)) {
        require(_swap != address(0), "Zero address: SWAP");
        SWAP = IERC20Extended(_swap);
    ...
    /**
     * @notice allow approved address to deposit an asset for SWAP
    ...
    function deposit(
        ...
        // mint SWAP needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        SWAP.transferFrom(address(0x0), msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn SWAP for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        SWAP.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new SWAP using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        SWAP.transferFrom(address(0x0), _recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or SWAP from the treasury, using sSWAP as collateral. This might allow an
     * sSWAP holder to provide SWAP liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(SWAP)) {
            require(permissions[STATUS.SWAPDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        // sSWAP.changeDebt(value, msg.sender, true);
        // require(sSWAP.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(SWAP)) {
            SWAP.transferFrom(address(0x0), msg.sender, value);
            swapDebt = swapDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20Extended(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20Extended(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        // sSWAP.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with SWAP
     * @param _amount uint256
     */
    function repayDebtWithSWAP(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.SWAPDEBTOR][msg.sender], notApproved);
        SWAP.burnFrom(msg.sender, _amount);
        // sSWAP.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        swapDebt = swapDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(SWAP), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SSWAP) {
            // sSWAP = IsSWAP(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SSWAP) {
            // 9
            // sSWAP = IsSWAP(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(SWAP.totalSupply().sub(totalDebt));
    }
    ...
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(SWAP)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return SWAP.totalSupply() - swapDebt;
    }
}
